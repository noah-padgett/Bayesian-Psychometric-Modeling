...
\begin{tikzpicture}[auto,scale=3,
	...
	manifest/.style={text centered, rectangle,draw,thick,inner sep=3pt,minimum height=8mm, minimum width=8mm, text width= 8 mm},
	  plate/.style={draw, shape=rectangle,thick, minimum height=4.25cm, minimum width=3.5cm, text width=1cm, align=right, inner sep=10pt, inner ysep=10pt, append after command={node[above left= 3pt of \tikzlastnode.south east] {#1}}},
...
\node [manifest] [above = 1.5cm of t, xshift=-0.75cm] (mt) {$\mu_T$};
\node [manifest] [above = 1.5cm of t, xshift=0.75cm] (st) {$\sigma^2_T$};
\node [manifest] [below = 1.5cm of x] (se) {$\sigma^2_E$};
