...
\[p(\bm{x}_i \vert \bm{\theta}, \bm{\pi}) = \prod_{i=1}^Np(\bm{x}_i \vert \theta_i=c, \bm{\pi}) = \prod_{i=1}^N\prod_{j=1}^Jp(x_{ij} \vert \theta_i=c, \pi_j).\]

The marginal probability when the observed data when we sum over the possible latent classes becomes

\[\begin{align*}
p(\bm{x}_i \vert \bm{\pi}, \bm{\gamma}) &= \prod_{i=1}^N\sum_{c=1}^C p(\bm{x}_i \vert \theta_i=c, \bm{\pi})p(\theta_i = c \vert \gamma)\\
...
```{r chp13-dag-1, echo=FALSE,fig.align='center',fig.cap='DAG for a general latent class analysis model', out.width="90%"}
...
\[\begin{align*}
p(\bm{x}\vert \bm{\theta}, \bm{\pi})&= \prod_{i=1}^N p(\bm{x}_i\vert \theta_i, \bm{\pi}) = \prod_{i=1}^N \prod_{j=1}^J p(x_{ij}\vert \theta_i, \bm{\pi}_j),\\
...
\[p(\theta)=\prod_{i=1}^np(\theta_i\vert\bm{\theta}_p),\]
where $\bm\theta_p$ represents the hyperpriors defining the conditions of the categorical latent variables.
The vector of parameters $\bm\theta_p = \bm\gamma = (\gamma_1, \gamma_2, ..., \gamma_C)$, where $C$ is the number of latent groups.
Stated in another way,
\[\theta_i | \bm\gamma \sim \mathrm{Categorical}(\bm\gamma).\]

The hyperprior for the categorical latent variable is also commonly given a prior distribution.
The $\bm\gamma$ parameters represent the class proportions and a prior on these parameters is useful when these class proportions are not known.
The prior is
\[\bm\gamma \sim \mathrm{Dirichlet}(\bm\alpha_\gamma),\]
where $\bm\alpha_\gamma = (\alpha_{\gamma 1}, \alpha_{\gamma 2}, ..., \alpha_{\gamma C}).$
...
The priors for the measurement model parameters ($\bm\pi$) are commonly defined at the item level instead of jointly over all items:
\[p(\bm\pi) = \prod_{j=1}^Jp(\bm\pi_j\vert\bm\alpha_\pi),\]
where $\bm\alpha_\pi$ defines the hyperpriors for the measurement model parameters.
...
\[
\begin{align*}
p(\bm\theta, \bm\gamma, \bm\pi) &\propto p(\mathbf{x} \vert \bm\theta, \bm\gamma, \bm\pi) p(\bm\theta, \bm\gamma, \bm\pi)\\
 &= p(\mathbf{x} \vert \bm\theta, \bm\gamma, \bm\pi) p(\bm\theta) p(\bm\gamma) p(\bm\pi)\\
 &= \prod_{i=1}^N \prod_{j=1}^J p(x_{ij}\vert \theta_i, \bm{\pi}_j)p(\theta_i \vert \bm\gamma)p(\bm\gamma) \prod_{c=1}^Cp(\bm\pi_{cj})
\end{align*}
\]
